...
3. **Parallel Scavenge（-XX:+UseParallelGC）**
   ...
   停顿时间越短越适合于与用户进行交互的程序，良好的响应速度更能提升用户体验，而高吞吐量则可以高效利用CPU时间，尽快完成程序的运算任务，主要适合在后台运算而不需要交互的任务。
...
3. **CMS（-XX:+UseConcMarkSweepGC）**
   ...
   **初始标记**（`CMS initial mark`）和 **重新标记**（`CMS remark`）会导致 **用户线程** 卡顿，`Stop the World` 现象发生。在整个过程中，`CMS` 回收器的 **内存回收** 基本上和 **用户线程** 并发执行。
...
2. [JVM（九）内存分配策略](https://blog.csdn.net/liupeifeng3514/article/details/79183734https://)
